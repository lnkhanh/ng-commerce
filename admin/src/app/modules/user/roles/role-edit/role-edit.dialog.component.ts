  ...
  loadPermissions() {
    this.allPermissions$.subscribe((_allPermissions) => {
      ...
      mainPermissions.forEach((element: Permission) => {
        const hasUserPermission = this.role.permissions.some(
          (t) => t === element.id
        );
        ...
        const children = _allPermissions.filter(
          (el) => el.parentId && el.parentId === element.id
        );
        children.forEach((child) => {
          const hasUserChildPermission = this.role.permissions.some(
            (t) => t === child.id
          );
  ...
  isSelectedChanged($event, permission: Permission) {
    if (permission._children.length === 0 && permission.isSelected) {
      const _root = find(
        ...
        (item: Permission) => item.id === permission.parentId
      );
    ...
    if (permission._children.length === 0 && !permission.isSelected) {
      ...
      if (_root && _root.isSelected) {
        if (
          !some(_root._children, (item: Permission) => item.isSelected === true)
        ) {
    ...
    if (permission._children.length > 0 && permission.isSelected) {
      each(
        permission._children,
        (item: Permission) => (item.isSelected = true)
      );
      return;
    }
